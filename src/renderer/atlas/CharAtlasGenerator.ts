...
export function generateStaticCharAtlasTexture(context: Window, canvasFactory: (width: number, height: number) => HTMLCanvasElement, config: ICharAtlasConfig): HTMLCanvasElement | Promise<ImageBitmap> {
  ...
  for (let i = 0; i < 256; i++) {
    ...
    ctx.clip();
    ctx.fillText(String.fromCharCode(i), i * cellWidth, cellHeight / 2, this.cellWidth);
    ctx.restore();
  }
  // Default color bold
  ctx.save();
  ...
  for (let i = 0; i < 256; i++) {
    ...
    ctx.clip();
    ctx.fillText(String.fromCharCode(i), i * cellWidth, cellHeight * 1.5, cellWidth);
    ctx.restore();
  }
  ctx.restore();
  ...
  for (let colorIndex = 0; colorIndex < 16; colorIndex++) {
    ...
    for (let i = 0; i < 256; i++) {
      ...
      ctx.fillText(String.fromCharCode(i), i * cellWidth, y + cellHeight / 2, cellWidth);
      ctx.restore();
    }
  }
